...
def test_par_generate_video_from_image(mock_par_execute, mock_generate, video_engine):
    ...
    mock_par_execute.assert_called_once_with(
        video_engine.generate_video_from_image, image_paths, video_paths, durations
    )
...
def test_concatenate_videos(
    ...
    # Verify file writes
    expected_writes = ["file '/absolute/video1.mp4'\n", "file '/absolute/video2.mp4'\n"]
    ...
    mock_ffmpeg.input.assert_called_once_with(
        "/tmp/temp_file.txt", format="concat", safe=0
    )
...
@patch("src.slide_to_video.video_engine.os.remove")
@patch("src.slide_to_video.video_engine.os.path.abspath")
def test_concatenate_videos_single_video(
    mock_abspath,
    mock_remove,
    mock_temp_file,
    mock_run_ffmpeg,
